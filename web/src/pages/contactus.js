...
const ContactusPage = () => {
    ...
    returValue = socialMedia.map((item, index)=>{
        switch (item.type) {
            ...
            case "contactus":
                return <SocialMedia key={index} href={item.url} target="_blank" >
                          <SocialMediaTitle>
                               <FaHandshake size='32' color='white' />&nbsp;Conozcámonos
                          </SocialMediaTitle>      
                       </SocialMedia> 
            default:
                return <SocialMedia />
  ...
  return (<PageContainer title="Contáctanos"
                        description="Diseñamos sitios web, aplicaciones móviles (iOS y Android) y automatizamos procesos. Usamos la nube de Amazon (AWS) para tus proyectos."
                        lang={Languages.Spanish}
                        activeMenu={ MenuToolBar.ContactUs } >
              <ContainerGetInTouch>
                    <GetInTouchBody>
                          <GetInTouchDescription>
                              <GetInTouchTitle>Estemos en Contacto</GetInTouchTitle>
                              Nos gustaría saber más sobre ti. Contáctanos cuando quieras
                          </GetInTouchDescription>
                          <GetInTouchButtons>
